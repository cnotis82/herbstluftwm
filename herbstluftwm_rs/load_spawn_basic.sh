...
spawn_with_rules() {(
    ...
    hc_rs rule pid=$BASHPID maxage=120 "${RULES[@]}"
    exec "$@"
    ) &
}

hc_rs chain , emit_hook no_bsp , add music , load music "(split vertical:0.8:0 (clients grid:0) (clients grid:0))" , emit_hook bsp
